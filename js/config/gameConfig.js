...
const WAVE_SYSTEM = {
  ...
  generateStandardWave(waveNumber) {
    ...
    const healthMultiplier = Math.min(
      rules.healthScaling.base + (waveNumber - 1) * rules.healthScaling.increment,
      rules.healthScaling.maxMultiplier
    );

    const speedMultiplier = Math.min(
      rules.speedScaling.base + (waveNumber - 1) * rules.speedScaling.increment,
      rules.speedScaling.maxMultiplier
    );

    const fireRateMultiplier = Math.min(
      rules.fireRateScaling.base + (waveNumber - 1) * rules.fireRateScaling.increment,
      rules.fireRateScaling.maxMultiplier
    );

    const enemyCount = Math.min(
      rules.enemyCountScaling.base + Math.floor((waveNumber - 1) / 2) * rules.enemyCountScaling.increment,
    ...
    if (waveNumber >= 3) {
      composition.interceptor = Math.min(0.2, 0.1 + (waveNumber - 3) * 0.02);
      composition.scout = Math.max(0.5, 0.7 - (waveNumber - 3) * 0.03);
    }

    if (waveNumber >= 2) {
      composition.tank = Math.min(0.3, 0.2 + (waveNumber - 2) * 0.02);
    }

    // Select formation based on wave number
    const formations = Object.keys(FORMATION_PATTERNS);
    const formationIndex = Math.floor((waveNumber - 1) / 3) % formations.length;
    ...
    return {
      ...
      composition,
      formation,
      spawnDelay: Math.max(50, base.spawnDelay - (waveNumber - 1) * 5),
      scaling: {
        health: healthMultiplier,
        speed: speedMultiplier,
        fireRate: fireRateMultiplier
      },
      difficultyMultiplier: base.difficultyMultiplier + (waveNumber - 1) * 0.1,
      bonusMultiplier: 1.0
    };
  },
